...
EUCLIDTRACK is an experimental synth workstation for generating melodies and rhythms through euclidean sequencing and modulation. Four tracks, each with its own synth voice, euclidean sequencer, and effect chain, all driven from the Brick's buttons, drawing from a deep collection of engines spanning virtual analog, FM, physical models, sample romplers, wavetables and drum machines. Sixty four scenes, four modulators per track, twelve performance macros, generous saturation with many drive algorithms, a hybrid morphing delay/reverb with a resonator and a shimmer stage, a master bus live looper, and a mixer that lets you build a feedback network for unique organismic (and slightly dangerous) results!
...
| left stick (Brick Pro / Smart Pro) | mirrors the dpad, layer combos included | |
| right stick (Brick Pro / Smart Pro) | dials the focused param with no hold: left/right fine, up/down coarse | |
...
2. **Hold South <img src="https://raw.githubusercontent.com/boorch/euclidtrack-pak/main/.github/icons/uF0E34.svg" height="16" alt="south"> and press the dpad** to change the value. Left and right are fine steps, up and down are coarse. On a Brick Pro or Smart Pro, the right stick does the same with no hold.
...
- **TRIGGERS** picks the trigger mode, which sets how long each hit sustains. **TRIGGER** plucks: the note strikes and decays away at once, percussive. **GATE** holds the note at full level for the whole step and only lets LPG DECAY take it down once the step is over, so a slower RATE rings longer; back-to-back hits re-strike from silence, each carrying its own pitch. **TIE** holds across the whole run of back-to-back hits: one strike, full level to the next rest, pitch unchanged the whole way, decay at the end of the run. Engines that shape their own envelopes (the drums, Modal, String, the 6op FM banks, Hyper Chords, Speech, the River family and the Greed family) sound the same in TRIGGER and GATE. TIE is different on them: a held gate is a single rising edge, so a run of back-to-back hits strikes once and the rest of the run stays silent.
