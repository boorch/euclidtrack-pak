...
The first three options are saved with the project (a template project carries them into every new project). THEME, QUICK MODULATION ASSIGN, SWAP FACE KEYS, LOAD QUANTIZE, and STICKS AS MACROS are saved globally, across every project and boot. KEEP BPM ON LOAD is the exception: session only, always off at boot and never written to disk, so you flip it on by hand each time you want it for a live set.
